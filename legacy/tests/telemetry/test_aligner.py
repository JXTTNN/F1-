...
class TestGridAndInterpolation:
    ...
    def test_motion_fields_interpolated(self) -> None:
        # g-force 已在 parse_motion ÷1000 → G 单位浮点，线性插值; world 坐标仍 float 线性插值。
        aligner = TelemetryAligner()
        ...
        assert f["g_lat"] == pytest.approx(2.2, abs=1e-6)  # 线性插值: 1.0→3.0 @1.6
        assert isinstance(f["g_lat"], float)
