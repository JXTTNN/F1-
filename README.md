...
- **反馈生命周期**：生成建议后自动清除本赛道反馈，避免跨圈混淌（点击“生成调教”后反馈将被消费，下一圈请重新录入）。
- **遥测训练 surrogate 模型**：引入 Ridge/MLP 双模型预测（R²=0.8909 / 0.5546），基于角点遥测数据，支持性能模型预测与调教建议。
- **假绿/失效检查修正**：修正参数项数（21→20）、强度区间（0-5→1-3）与测试断言，所有测试均通过（2110 passed / 7 skipped）。
