...
def _report_per_lap(r: dict) -> tuple[list[str], int, int, bool, bool]:
    ...
    return lines, len(violations), len(missing)
...
def generate_report(results: list[dict]) -> str:
    ...
    total_v = total_m = 0
    for r in results:
        lap_lines, nv, nm = _report_per_lap(r)
        lines.extend(lap_lines)
        total_v += nv
        total_m += nm
    lines.extend(_report_summary(total_v, total_m))
    return "\n".join(lines)


def _report_summary(total_v: int, total_m: int) -> list[str]:
    """格式化汇总段。"""
