﻿# 测试 VBS 直接运行 F1OPT.exe（不用cmd/BAT中间层）
$distDir = "D:\F1OPT-Test\dist"

Write-Host "=== VBS直接运行exe测试 ===" -ForegroundColor Cyan
...
"@
$testVbsPath = "$distDir\test_direct_exe.vbs"
$testVbs | Out-File -FilePath $testVbsPath -Encoding ASCII

Write-Host "[1] VBS直接运行F1OPT.exe..." -ForegroundColor Yellow
$startTime = Get-Date
