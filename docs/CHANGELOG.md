...
- **用户诉求**："前一圈的车手反馈在点击生成优化后调教要清除，要不然杂着后一圈吗"。
  ...
  影响，而 F1 针对的问题已被上一轮调教修正 —— 混入即噪声。
- **修复**：`POST /api/v1/suggest` 新增 `clear_feedback_after_suggest`（默认 `True`）：
  生成**成功后**清除本赛道反馈并如实返回"已清除 N 条"。时序为**落库成功之后、
  WS 推送之前**（前端收到 WS 事件即刷新不会读回旧数据）；生成/落库失败时保留
  反馈供重试；清除失败非致命（建议已落库，记日志不 500）。设 `False` 保留累积
  语义（连续重新生成 / 性能基准等）。`Store.clear_track_feedback` 返回删除条数，
  空赛道返回 0（幂等）。
- **前端**（`ui/app.js`）：生成成功与 WS 事件后自动刷新反馈摘要与赛道图"已反馈"
  标记；`loadExistingFeedback` 改为**先重置旧标记再重标**（原实现只累加，清除后
  赛道图残留已反馈圆点）。
- **假绿测试修正**：`test_increasing_strength_increases_delta_magnitude` 原用
  `strength=5`（合法区间 [1,3]，HTTP 422 被静默忽略）—— "强度更大"比较的是同一份
  输入。现改合法上限 3、同一弯道，并**断言每次 POST 状态码**，杜绝静默失败。
- **失效检查修正**：`api_endpoint_test.py` 3 处过时断言（21 项参数 → 实际 20 项）
  与非法 `strength: 4`；`run_local_test.py` 同类 `strength: 4`；`perf_benchmark.py` /
  `perf_analysis.py` 连续生成基准须显式关闭清除（否则第 2 次起测到 400 错误路径）。
  修正后端点全检 **23 passed / 0 failed**。
- **验证**：新增 `tests/test_feedback_lifecycle.py` 7 条（默认清除 / 关闭保留 /
  **跨圈纯净性**：两轮结果 == 全新环境仅含第二轮反馈，逐位一致 / 分赛道隔离 /
  store 幂等）；负向验证两轮（停掉清除、还原"读前清除"旧 bug → 3 用例变红）。
  `pytest -q` **2110 passed**；`ruff` 全绿。
