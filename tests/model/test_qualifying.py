...
# Helpers
# --------------------------------------------------------------------------- #
def _make_drivers(n: int = 20) -> list[DriverQualifyingInput]:
    return [
        DriverQualifyingInput(
            driver_id=f"d{i:02d}",
...
class TestBasicStructure:
    def test_returns_20_results(self) -> None:
        grid = simulate_qualifying("monza", _make_drivers(20), seed=42)
        assert len(grid) == 20

    def test_grid_positions_1_to_20(self) -> None:
        grid = simulate_qualifying("monza", _make_drivers(20), seed=42)
...
class TestElimination:
    def test_5_drivers_eliminated_q1(self) -> None:
        grid = simulate_qualifying("monza", _make_drivers(20), seed=42)
        q1_out = [r for r in grid if r.eliminated_in == "Q1"]
        assert len(q1_out) == 5
        # All in P16-P20
        for r in q1_out:
            assert 16 <= r.grid_position <= 20

    def test_5_drivers_eliminated_q2(self) -> None:
        grid = simulate_qualifying("monza", _make_drivers(20), seed=42)
        q2_out = [r for r in grid if r.eliminated_in == "Q2"]
        assert len(q2_out) == 5
        for r in q2_out:
            assert 11 <= r.grid_position <= 15
...
class TestSkillCorrelation:
    def test_top_drivers_usually_top_grid(self) -> None:
        ...
        q3_count = 0
        for seed in range(5):
            grid = simulate_qualifying("monza", _make_drivers(20), seed=seed)
...
class TestReproducibility:
    def test_same_seed_same_result(self) -> None:
        g1 = simulate_qualifying("monza", _make_drivers(20), seed=42)
        g2 = simulate_qualifying("monza", _make_drivers(20), seed=42)
    ...
    def test_different_seed_different_result(self) -> None:
        g1 = simulate_qualifying("monza", _make_drivers(20), seed=42)
        g2 = simulate_qualifying("monza", _make_drivers(20), seed=99)
...
class TestTrackSpecific:
    def test_monza_lap_faster_than_monaco(self) -> None:
        ...
        g_monza = simulate_qualifying("monza", _make_drivers(20), seed=42)
        g_monaco = simulate_qualifying("monaco", _make_drivers(20), seed=42)
...
class TestEdgeCases:
    def test_minimum_drivers_q1_only(self) -> None:
        """If 20 drivers — Q1 still eliminates 5."""
        grid = simulate_qualifying("monza", _make_drivers(20), seed=1)
        assert len(grid) == 20
    ...
    def test_validation_too_few_drivers(self) -> None:
        """Currently no validation; with 15 drivers Q3 still gets 10."""
