...
class TestCalibrationCurve:
    def test_perfect_prediction(self) -> None:
        ...
        x = np.random.randn(100, 41).astype(np.float32)
    ...
    def test_small_dataset(self) -> None:
        ...
        x = np.random.randn(10, 41).astype(np.float32)
    ...
    def test_constant_y(self) -> None:
        ...
        x = np.random.randn(20, 41).astype(np.float32)
        y = np.ones(20) * 85.0
        r = calibration_curve(m, x, y)
...
class TestPerTrackBreakdown:
    def test_basic(self) -> None:
        ...
        x = np.random.randn(60, 41).astype(np.float32)
...
class TestPredictionUncertainty:
    ...
    def test_single_point(self) -> None:
        ...
        x = np.random.randn(1, 41).astype(np.float32)
...
class TestFeatureImportance:
    def test_basic_importance(self) -> None:
        ...
        assert len(result['names']) == 41
        assert len(result['importance_mean']) == 41
        assert len(result['importance_std']) == 41
    ...
    def test_custom_feature_names(self) -> None:
        ...
        x, sec_y, resp_y, sp, rp = _build_tensors(data)
        y_lap = np.asarray(sec_y.sum(dim=1))
        names = ['fw_angle', 'rw_angle'] + [f'f{i}' for i in range(2, 41)]
